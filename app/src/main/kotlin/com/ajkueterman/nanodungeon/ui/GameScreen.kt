...
fun GameScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    val depth = (state as? GameUiState.Exploring)?.depth
                    Text(if (depth != null) "Room $depth" else "Nano Dungeon")
                },
        ...
        Box(Modifier.fillMaxSize().padding(padding)) {
            when (state) {
                GameUiState.Starting -> CenteredMessage("You descend into the dark…", showSpinner = true)
    ...
    Column(
        ...
        AnimatedContent(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                SceneCard(scene)
                // One button per generated Choice. The schema guarantees exactly two.
                scene.choices.forEach { choice ->
                    ChoiceButton(choice, enabled = !state.isThinking, onClick = { onChoose(choice) })
                }
            }
        }
        if (state.isThinking) {
 ...
 */
@Composable
fun SceneCard(scene: Scene, modifier: Modifier = Modifier) {
    ...
    Card(
        ...
        Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text(scene.narration, style = MaterialTheme.typography.bodyLarge)
        }
    }
}
...
@Composable
fun ChoiceButton(choice: Choice, enabled: Boolean, onClick: () -> Unit, modifier: Modifier = Modifier) {
    OutlinedButton(
        onClick = onClick,
        enabled = enabled,
        modifier = modifier.fillMaxWidth(),
...
// A hand-written Scene. The model's output has exactly this shape, so any Scene works here.
private val PreviewScene = Scene(
    ...
    mood = MOOD_EERIE,
    choices = listOf(
        Choice("Follow the rushing water", "The roar grows louder, and colder air drifts back."),
        Choice("Squeeze into the side passage", "A smell of wet moss and turned earth."),
    ),
)
...
private fun GameScreenPreview() {
    NanoDungeonTheme {
        GameScreen(GameUiState.Exploring(PreviewScene, depth = 3), onChoose = {}, onRetry = {}, onNewRun = {})
    }
}
...
private fun GameScreenThinkingPreview() {
    NanoDungeonTheme {
        GameScreen(
            GameUiState.Exploring(PreviewScene, depth = 3, isThinking = true),
